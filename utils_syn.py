...
def generate_data():
    mean = config_syn.mu
    covariance = config_syn.cov
    data = np.random.multivariate_normal(mean, covariance, config_syn.T_train + config_syn.T_test + config_syn.T_truetest)
...
def missing(miss_prob=0.5):
    ...
    mask = np.random.choice([True, False], size = (config_syn.T_train,config_syn.n), p = [miss_prob, 1-miss_prob])
    mask = np.vstack((mask, np.full((config_syn.T_test + config_syn.T_truetest,config_syn.n),False))) #no missing value for testing period
    return mask
...
def individualposterior(data, mask, mu0, cov0_inv):
    ...
    num_posteriors = config_syn.T_test + 1#number of posteriors, from T_train to T_train+T_test
    ...
    for t in range(config_syn.T_train + config_syn.T_test):
        
        if not np.all(mask[t,:] == np.full(config_syn.n,True)):#except for all missing values at the t-th sample, otherwise run the following
            
            ind = (mask[t,:] == np.full(config_syn.n,False))#judge each element of the t-th sample missing or not
            cov_t = config_syn.cov[np.ix_(ind,ind)]#cov_t = cov[non-missing entries], i.e. Omega_Xt
            cov_inv_expand = np.zeros((config_syn.n,config_syn.n))
            cov_inv_expand[np.ix_(ind,ind)] = np.linalg.inv(cov_t)#(P_Mt)(Omega_Xt)

            covariance_mu_inv += cov_inv_expand#sum_{Tk}((P_Mt)^{-1}(Omega_Xt))

            mean_t = np.zeros(config_syn.n)
            mean_t[ind] = data[t,ind]#mean_t = ((P_Mt)^{-1}(Xt))
            mean_mu += np.matmul(cov_inv_expand,mean_t)#sum_{Tk}((P_Mt)^{-1}(Omega_Xt))((P_Mt)^{-1}(Xt))
        
        if t >= (config_syn.T_train - 1):#when t>= T_train-1, each for-loop generates a new (cov_k,mu_k). Put (cov_k,mu_k) into list, and we have a list of covariance and mean of posteriors.
            covariance_mu = np.linalg.inv(covariance_mu_inv)#cov_k
...
def consensusforwardkl(meanlist,covariancelist,delta_r,prediction,rho):
    ...
    config_syn.n = len(meanlist[0])#number of stocks
    # Define optimization variables
    weights = cp.Variable(num_posteriors)
    gamma = cp.Variable(config_syn.n)
    ...
    for i in range(num_posteriors):
        s_temp = np.zeros(config_syn.n)
        for j in range(config_syn.n):
    ...
    c = np.zeros((num_posteriors,config_syn.n))#ckj stores in c
    ...
    #objective_fun = [cp.power(cp.sum(cp.multiply(inverse_sv[:,j], weights)),-1) for j in range(config_syn.n)]
    
    obj = cp.sum([gamma[j] for j in range(config_syn.n)])#sum(gamma_j)
    
    # Run optimization
    objective = cp.Minimize(obj)
    delta = delta_r * max([np.abs(c[-1,j]/inverse_sv[-1,j] - v[:,j].dot(prediction)) for j in range(config_syn.n)])
...
def consensuswasserstein(meanlist,covariancelist,delta_r,prediction,rho,set_choice):
    ...
    final_sigma = (solution[0]*np.eye(config_syn.n) + solution[1] * Psi) @ Sigma1 @ (solution[0]*np.eye(config_syn.n) + solution[1] * Psi)
...
def consensuswasserstein_general(meanlist,covariancelist,delta_r, prediction, rho, set_choice):
    ...
    obj = np.inner(weights, np.matmul(P,weights))
    ...
    final_mu = np.zeros(config_syn.n)
    for i in range(num_posteriors):
        final_mu += solution[i]*meanlist[i]
    final_sigma = np.zeros((config_syn.n,config_syn.n))
    for i in range(num_posteriors):
        for j in range(config_syn.n):
...
def consensusbackwardkl(meanlist,covariancelist,delta_r,prediction,weight_init,epsilon,max_iter,rho,set_choice):#weight_init = np.full(num_posteriors,1/num_posteriors)
    ...
    while np.linalg.norm(weight_t-weight_former) > epsilon and iteration < max_iter:
        ...
        if set_choice == 'Vz-l1-norm':
            constraints.append(cp.norm(v @ (temp-prediction),"inf") <= delta/rho)
        #temp = 0
        #for j in range(config_syn.n): #j:0,...config_syn.n-1
        #    for i in range(num_posteriors):
    ...
    final_sigma = np.zeros((config_syn.n,config_syn.n))
    final_sigma_1 = np.zeros((config_syn.n,config_syn.n))
    for i in range(num_posteriors):
...
def imputation(data, mask, final_mu,final_sigma):    
    config_syn.m = 10 #number of multiply-imputed dataset
    total_time,num_stocks = data.shape
    completed_data = np.zeros((config_syn.m,total_time,num_stocks))
    for k in range(config_syn.m):
        data_copy = np.copy(data)
        unconditionalmean = np.random.multivariate_normal(final_mu, final_sigma)
        for t in range(config_syn.T_train):
            if np.all(mask[t,:] == np.full(config_syn.n,True)):
                data_copy[t,:] = unconditionalmean
            elif np.all(mask[t,:] == np.full(config_syn.n,False)):
                pass
            else:
                ind_miss = (mask[t,:] == np.full(config_syn.n,True))
                ind_obs = (mask[t,:] == np.full(config_syn.n,False))
                len_miss = len(data_copy[t,ind_miss])
                data_copy[t,ind_miss] = np.zeros(len_miss)
                cov11 = config_syn.cov[np.ix_(ind_miss,ind_obs)]
                cov12_inv = np.linalg.inv(config_syn.cov[np.ix_(ind_obs,ind_obs)])
                ...
                missing_condi_cov = config_syn.cov[np.ix_(ind_miss,ind_miss)] - np.matmul(cov11,np.matmul(cov12_inv,np.matrix.transpose(cov11)))
...
def Greedy(data1):
    data = np.copy(data1)
    data = data/100
    mean = np.mean(np.exp(data[:config_syn.T_train,:])-1.0,axis = 0)
    #if np.sum(mean) <= 0:
    #    print('whoops')
    weights = mean / np.linalg.norm(mean,2)
    #c_returns = 1.0
    returns = np.zeros(config_syn.T_test)
    for t in np.arange(config_syn.T_train,config_syn.T_train+config_syn.T_test,1):
        returns[t-config_syn.T_train] = np.inner(np.exp(data[t,:])-1.0, weights)
        #c_returns *= 1.0 + returns[t-config_syn.T_train]
    sharper = np.mean(returns)/np.std(returns)
    
    #o_returns = 1.0
    returns_o = np.zeros(config_syn.T_truetest)
    for t in np.arange(config_syn.T_train+config_syn.T_test,config_syn.T_train+config_syn.T_test+config_syn.T_truetest,1):
        returns_o[t-config_syn.T_train - config_syn.T_test] = np.inner(np.exp(data[t,:])-1.0, weights)
       # o_returns *= 1.0 + returns_o[t-config_syn.T_train - config_syn.T_test]
