...
def missing(miss_prob=0.4):
    ...
    mask = np.random.choice([True, False], size = (config_ip.T_train,config_ip.n), p = [miss_prob, 1-miss_prob])#40% probability of missing a value
    mask = np.vstack((mask, np.full((config_ip.T_test + config_ip.T_truetest,config_ip.n),False))) #no missing value for testing period
    return mask#mask is a True, False matrix about missing

def individualposterior(data, mask, mu0, cov0_inv, cov, num_points = 50):#the maximum of number of posteriors is T_test+1
    ...
    num_posteriors = config_ip.num_points + 1 #number of posteriors, from T_train to T_train+T_test
    ...
    for t in range(config_ip.T_train + config_ip.T_test):
        
        if not np.all(mask[t,:] == np.full(config_ip.n,True)):#except for all missing values at the t-th sample, otherwise run the following
            ind = (mask[t,:] == np.full(config_ip.n,False))#judge each element of the t-th sample missing or not
            cov_t = config_ip.cov[np.ix_(ind,ind)]#cov_t = cov[non-missing entries], i.e. Omega_Xt
            cov_inv_expand = np.zeros((config_ip.n,config_ip.n))
            ...
            mean_t = np.zeros(config_ip.n)
            mean_t[ind] = data[t,ind]#mean_t = ((P_Mt)^{-1}(Xt))
            mean_mu += np.matmul(cov_inv_expand,mean_t)#sum_{Tk}((P_Mt)^{-1}(Omega_Xt))((P_Mt)^{-1}(Xt))
        
        if t >= (config_ip.T_train - 1):#when t>= T_train-1, each for-loop generates a new (cov_k,mu_k). Put (cov_k,mu_k) into list, and we have a list of covariance and mean of posteriors.
            if (t - config_ip.T_train + 1) % (config_ip.T_test/config_ip.num_points) == 0:
...
def consensusforwardkl(meanlist,covariancelist,delta_r,prediction,rho):
    ...
    config_ip.n = len(meanlist[0])#number of stocks
    # Define optimization variables
    weights = cp.Variable(num_posteriors)
    gamma = cp.Variable(config_ip.n)
    ...
    for i in range(num_posteriors):
        s_temp = np.zeros(config_ip.n)
        for j in range(config_ip.n):
    ...
    c = np.zeros((num_posteriors,config_ip.n))#ckj stores in c
    ...
    #objective_fun = [cp.power(cp.sum(cp.multiply(inverse_sv[:,j], weights)),-1) for j in range(config_ip.n)]
    
    obj = cp.sum([gamma[j] for j in range(config_ip.n)])#sum(gamma_j)
    
    # Run optimization
    objective = cp.Minimize(obj)
    delta = delta_r * max([np.abs(c[-1,j]/inverse_sv[-1,j] - v[:,j].dot(prediction)) for j in range(config_ip.n)])
...
def consensuswasserstein(meanlist,covariancelist,delta_r,prediction,rho,set_choice):
    ...
    final_sigma = (solution[0]*np.eye(config_ip.n) + solution[1] * Psi) @ Sigma1 @ (solution[0]*np.eye(config_ip.n) + solution[1] * Psi)
...
def consensuswasserstein_general(meanlist,covariancelist,delta_r, prediction, rho, set_choice):
    ...
    final_mu = np.zeros(config_ip.n)
    for i in range(num_posteriors):
        final_mu += solution[i]*meanlist[i]
    final_sigma = np.zeros((config_ip.n,config_ip.n))
    for i in range(num_posteriors):
        for j in range(config_ip.n):
...
def consensusbackwardkl(meanlist,covariancelist,delta_r,prediction,weight_init,epsilon,max_iter,rho,set_choice):#weight_init = np.full(num_posteriors,1/num_posteriors)
    ...
    while np.linalg.norm(weight_t-weight_former) > epsilon and iteration < max_iter:
        ...
        if set_choice == 'Vz-l1-norm':
            constraints.append(cp.norm(v @ (temp-prediction),"inf") <= delta/rho)
        #temp = 0
        #for j in range(config_ip.n): #j:0,...config_ip.n-1
        #    for i in range(num_posteriors):
    ...
    final_sigma = np.zeros((config_ip.n,config_ip.n))
    final_sigma_1 = np.zeros((config_ip.n,config_ip.n))
    for i in range(num_posteriors):
...
def imputation(data, mask, final_mu,final_sigma):    
    config_ip.m = 100 #number of multiply-imputed dataset
    total_time,num_stocks = data.shape
    completed_data = np.zeros((config_ip.m,total_time,num_stocks))
    for k in range(config_ip.m):
        data_copy = np.copy(data)
        unconditionalmean = np.random.multivariate_normal(final_mu, final_sigma)
        for t in range(config_ip.T_train):
            if np.all(mask[t,:] == np.full(config_ip.n,True)):
                data_copy[t,:] = unconditionalmean
            elif np.all(mask[t,:] == np.full(config_ip.n,False)):
                pass
            else:
                ind_miss = (mask[t,:] == np.full(config_ip.n,True))
                ind_obs = (mask[t,:] == np.full(config_ip.n,False))
                len_miss = len(data_copy[t,ind_miss])
                data_copy[t,ind_miss] = np.zeros(len_miss)
                cov11 = config_ip.cov[np.ix_(ind_miss,ind_obs)]
                cov12_inv = np.linalg.inv(config_ip.cov[np.ix_(ind_obs,ind_obs)])
                ...
                missing_condi_cov = config_ip.cov[np.ix_(ind_miss,ind_miss)] - np.matmul(cov11,np.matmul(cov12_inv,np.matrix.transpose(cov11)))
...
def Greedy(data1):
    data = np.copy(data1)
    data = data/100
    mean = np.mean(np.exp(data[:config_ip.T_train,:])-1.0,axis = 0)
    #if np.sum(mean) <= 0:
    #    print('whoops')
    weights = mean / np.linalg.norm(mean,2)
    #c_returns = 1.0
    returns = np.zeros(config_ip.T_test)
    for t in np.arange(config_ip.T_train,config_ip.T_train+config_ip.T_test,1):
        returns[t-config_ip.T_train] = np.inner(np.exp(data[t,:])-1.0, weights)
        #c_returns *= 1.0 + returns[t-config_ip.T_train]
    sharper = np.mean(returns)/np.std(returns)
    
    #o_returns = 1.0
    returns_o = np.zeros(config_ip.T_truetest)
    for t in np.arange(config_ip.T_train+config_ip.T_test,config_ip.T_train+config_ip.T_test+config_ip.T_truetest,1):
        returns_o[t-config_ip.T_train - config_ip.T_test] = np.inner(np.exp(data[t,:])-1.0, weights)
       # o_returns *= 1.0 + returns_o[t-config_ip.T_train - config_ip.T_test]
