...
def missing(miss_prob=0.4):
    ...
    mask = np.random.choice([True, False], size = (config_rs.T_train,config_rs.n), p = [miss_prob, 1-miss_prob])#40% probability of missing a value
...
def individualposterior(data, mask, mu0, cov0_inv, cov, num_points = 50):#the maximum of number of posteriors is T_test+1
    ...
    for t in range(config_rs.T_train + config_rs.T_test):
        ...
        if t >= (config_rs.T_train - 1):#when t>= T_train-1, each for-loop generates a new (cov_k,mu_k). Put (cov_k,mu_k) into list, and we have a list of covariance and mean of posteriors.
            if (t - config_rs.T_train + 1) % (config_rs.T_test/config_rs.num_points) == 0:
...
def imputation(data, mask, final_mu,final_sigma):    
    config_rs.m = 100 #number of multiply-imputed dataset
