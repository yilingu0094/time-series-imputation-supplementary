...
def missing(miss_prob=0.5):
    ...
    mask = np.random.choice([True, False], size = (config_ip.T_train,config_ip.n), p = [miss_prob, 1-miss_prob])#50% probability of missing a value
...
def individualposterior(data, mask, mu0, cov0_inv, cov):#the maximum of number of posteriors is T_test+1
    ...
    for t in range(config_ip.T_train + config_ip.T_test):
        ...
        if t >= (config_ip.T_train - 1):#when t>= T_train-1, each for-loop generates a new (cov_k,mu_k). Put (cov_k,mu_k) into list, and we have a list of covariance and mean of posteriors.
            if (t - config_ip.T_train + 1) % (config_ip.T_test/num_posteriors) == 0:
